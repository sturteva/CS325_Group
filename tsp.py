...
def two_opt(tour_list, edge_list,start_time):
    ...
    while (current_time - start_time) * 1000 < 140000 and i < len(curbest):
        print i
        ...
        if v1 == v3 or v1 == v4 or v2 == v3 or v2 == v4:
            ...
            print 'got here'
            #odds/Even distance
            odd_distance = get_distance(v1,v3)
            even_distance = get_distance(v2,v4)
            odd_even = odd_distance + even_distance

            #inside/outside distance
            inside_distance = get_distance(v2,v3)
            outside_distance = get_distance(v1,v4)
            inside_outside = inside_distance + outside_distance

            #checks to see which is smaller
            if odd_even < inside_outside:
                print 'oddEVen smaller'
                #now checks if less than currentdistance
                if odd_even < target_distance:
...
def write_tour(o, tour_list, search_city):
...
if __name__ == '__main__':
    ...
    if (timeCheck - start_time) * 1000 < 140000:
    ...
    with open(sys.argv[1] + '.tour', 'w') as o:
        ...
        write_tour(o, tour_list, current_edge.v2)
